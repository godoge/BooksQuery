...
public class CategoryFactory {
    ...
    Category getACategory(Cursor cursor) {
        int index_name = cursor.getColumnIndex(DbConstance.CATEGORY_COLUMN_NAME);
        Category category = new Category();
        String name = cursor.getString(index_name);
        category.setName(name);
        category.setCount(dbTools.getKeyWordCount(DbConstance.BOOK_COLUMN_CATEGORY, name));
        return category;
    }

    public CategoryFactory setCategoryFromDb() {
        list.clear();
