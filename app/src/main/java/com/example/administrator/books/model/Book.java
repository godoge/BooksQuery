...
public class Book implements Sqlable {
    ...
    public void setLocalMap(Map<String, Object> map) {
        if (map == null)
            return;
        if (map.get(DbConstance.COLUMN_AVERATE) == null)
            average = "0";
        else
    ...
    public void setBookByMap(Map<String, String> map) {
        ...
        if (map.get(ApiConstants.JSON_TAGS) != null)
            tags = map.get(ApiConstants.JSON_TAGS);
        else
            tags = STR_DEFAULT;
        if (map.get(LOCAL_CATEGORY) != null)
            category = map.get(LOCAL_CATEGORY);
        else
            category = STR_DEFAULT;
