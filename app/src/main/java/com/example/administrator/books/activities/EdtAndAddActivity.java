...
public class EdtAndAddActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private final int RESQUE_CAMERA = 0;
    private final int RESQUE_IMAGE = 1;
    ...
    void initValue() {
        ...
        path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).getAbsolutePath() + "/pics/";
    ...
    private Boolean checkIsInputCompletion() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        switch (which) {
                            case 0:
                                ...
                                startActivityForResult(intent, RESQUE_CAMERA);
                                break;
                            case 1:
                                intent.setAction(Intent.ACTION_GET_CONTENT);
                                intent.setType("image/*");
                                startActivityForResult(intent, RESQUE_IMAGE);
                                break;
                        }
                    }
                }).show();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == RESQUE_CAMERA && resultCode != RESULT_OK) {
            File file = new File(tempFilePath);
            if (file.exists())
                file.delete();
            return;
        }
        if (requestCode == RESQUE_IMAGE && resultCode != RESULT_OK)
            return;
        File last_file = new File(filePath);
        if (last_file.exists())
            last_file.delete();
        if (requestCode == RESQUE_CAMERA)
            filePath = tempFilePath;
        if (requestCode == RESQUE_IMAGE) {
            try {
                fileName = System.currentTimeMillis() + ".jpg";
